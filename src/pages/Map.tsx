...
const Map = () => {
  ...
  // Auto-detect user location when page loads (if user is logged in)
  useEffect(() => {
    if (user && !userLocation) {
      getUserLocation();
    }
    // We intentionally do not include getUserLocation in deps to avoid
    // re-creating the handler; it is stable within this component.
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className="order-1 lg:order-2 space-y-4 max-h-[600px] overflow-y-auto pr-2">
              {pointsWithDistance.map((point) => {
                const distance = (
                  point as CollectionPoint & { distance?: number }
                ).distance;
                return (
                  <Card
                    key={point.id}
                  ...
                  >
                    <div className="flex items-start justify-between mb-4">
                      ...
                      {userLocation && typeof distance === "number" && (
